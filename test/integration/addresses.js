...
describe('bitcoinjs-lib (addresses)', function () {
  ...
  it('can generate a SegWit 2-of-2 multisig address (via P2SH)', function () {
    ...
    var redeemScript = bitcoin.script.multisig.output.encode(2, pubKeys) // 2 of 2
    var redeemScript2 = bitcoin.script.witnessScriptHash.output.encode(bitcoin.crypto.sha256(redeemScript))
    var scriptPubKey = bitcoin.script.scriptHash.output.encode(bitcoin.crypto.hash160(redeemScript2))
    var address = bitcoin.address.fromOutputScript(scriptPubKey)
  ...
  it('can generate a SegWit 3-of-4 multisig address', function () {
    ...
    var redeemScript = bitcoin.script.multisig.output.encode(3, pubKeys) // 3 of 4
    var scriptPubKey = bitcoin.script.witnessScriptHash.output.encode(bitcoin.crypto.sha256(redeemScript))
    var address = bitcoin.address.fromOutputScript(scriptPubKey)
